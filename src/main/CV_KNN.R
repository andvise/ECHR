...
KNN_Accuracy <- function(article_number) {
        ...
        for (section in sections) {
            ...
            for (l in 1:10) {
                ...
                fit_knn <- train(x = train_data,
                                 ...
                                 tuneGrid = knnGrid)
                # predictions
                prediction <- predict(object=fit_knn, test_data[,1:total_data])
...
#accuracy_section <- KNN_Accuracy("3")
#accuracy_section <- KNN_Accuracy("6")
accuracy_section <- KNN_Accuracy("8")
