...
path <- "~/projects/ECHR-judicial-decisions/data/Article8/"

average_tables <- array(dim = c(26,10,10,10), dimnames = list(NULL, NULL, labels, NULL))
...
for (i in 1:10) {
    ...
    for (section in sections) {
        ...
        # path
        if (section == "topics") {
            file <- "formatted_topics8.csv"
            total_data <- 30
        } else if (section == "topcir") {
            file <- "formatted_topicsAndCircumstances.csv"
            total_data <- 2030
        } else {
            file <- paste("formatted_ngrams_a8_", section, ".csv", sep="")
            total_data <- 2000
        }
        ...
        # 10 x 25 folds
        folds <- cut(seq(1, 254), breaks=10, labels=FALSE)
        ...
        for (l in 1:10) {
            ...
            else if (section == "circumstances") fit_clf = fit_knn
            ...
            else if (section == "topcir") fit_clf = fit_svm
            
            prediction <- predict(object=fit_clf, test_data[,1:total_data])
            if (length(prediction) == 25) {
