...
path <- "~/projects/ECHR-judicial-decisions/data/Article6/"

average_tables <- array(dim = c(8,10,10,10), dimnames = list(NULL, NULL, labels, NULL))
...
for (i in 1:10) {
    ...
    for (section in sections) {
        ...
        # path
        if (section == "topics") {
            file <- "formatted_topics6.csv"
            total_data <- 30
        } else if (section == "topcir") {
            file <- "formatted_topicsAndCircumstances.csv"
            total_data <- 2030
        } else {
            file <- paste("formatted_ngrams_a6_", section, ".csv", sep="")
            total_data <- 2000
        }
        ...
        # 10 x 25 folds
        folds <- cut(seq(1, 80), breaks=10, labels=FALSE)
        ...
        for (l in 1:10) {
            ...
            else if (section == "circumstances") fit_clf = fit_svm
            else if (section == "relevantLaw") fit_clf = fit_gbm
            else if (section == "facts") fit_clf = fit_gbm
            else if (section == "law") fit_clf = fit_svm
            else if (section == "topics") fit_clf = fit_knn
            else if (section == "topcir") fit_clf = fit_knn
